...
// A closure called from inside another closure is an upvalue. The compiler
// once handled only the local case and died with "unknown function".

fn main() {
    let lines_of = |word: &str, i: usize| -> String { format!("{word}:{i}") };

    // Direct call, the case that always worked.

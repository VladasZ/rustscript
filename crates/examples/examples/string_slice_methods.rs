...
// `s[range].method()` on a String. The writeback splices the slice back into the base, through
// cells, fields and other projections.
...
fn main() {
    // `replace` is on the mutating list for `Option::replace`, but string `replace` doesn't mutate
    ...
    // 2 chained projections, the annotation keeps this a vector, clippy would ask for an array

//! Item registration at load time. Walks the items of every module and fills the resolver, the
//! impl table and the pending function and const lists that `Interp::load` then compiles.
...
pub(super) fn from_source_name(imp: &syn::ItemImpl) -> Option<String> {
...
/// different impls.
pub(super) fn from_type_key(ty: &syn::Type) -> Option<String> {
    match ty {
        syn::Type::Path(p) => {
...
pub(super) fn impl_target(resolver: &Resolver, m: usize, ty: &syn::Type) -> Option<String> {
...
pub(super) fn foreign_impl_key(path: &syn::Path) -> String {
...
pub(super) fn quote_kind(item: &Item) -> &'static str {
    match item {
        Item::Fn(_) => "fn",

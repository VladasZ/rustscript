...
pub(super) fn ratatui_assoc(ty: &str, func: &str, args: &[Value]) -> Option<Value> {
    ...
    Some(match (ty, func) {
        ("Style", "new" | "default") => style_value(Style::new()),
        ("Rect", "new") => Value::struct_of(
            "Rect",
            [
                ("x".into(), arg(0)),
                ("y".into(), arg(1)),
        ...
        ("Block", "new" | "default") => block_value(false),
        ("Block", "bordered") => block_value(true),
        ("Sparkline", "default" | "new") => Value::struct_of(
            "Sparkline",
            [
...
pub(super) fn style_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    let arg = |i: usize| -> Value { args.get(i).cloned().unwrap_or(Value::Unit) };
    Ok(match name {
        "fg" => with(s, "fg", Value::some(arg(0))),
        "bg" => with(s, "bg", Value::some(arg(0))),
        "add_modifier" => with(s, "add_modifier", arg(0)),
        "remove_modifier" => with(s, "sub_modifier", arg(0)),
        "bold" => add_modifier(s, ratatui::style::Modifier::BOLD),
        "dim" => add_modifier(s, ratatui::style::Modifier::DIM),
        "italic" => add_modifier(s, ratatui::style::Modifier::ITALIC),
        "underlined" => add_modifier(s, ratatui::style::Modifier::UNDERLINED),
        "reversed" => add_modifier(s, ratatui::style::Modifier::REVERSED),
...
pub(super) fn modifier_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    ...
    Ok(match name {
        ...
        "is_empty" => Value::Bool(mine.is_empty()),
        "bits" => Value::Int(i64::from(mine.bits())),
        "union" => modifier_value(mine | other),
        "difference" => modifier_value(mine - other),
        _ => bail!("unknown method `{name}` on Modifier"),
    })
}

pub(super) fn span_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    Ok(match name {
        "width" => {
            let span = value_span(&rebuild(s));
            Value::Int(i64::try_from(span.width()).unwrap_or(0))
        }
        "style" => with(s, "style", args.first().cloned().unwrap_or(Value::Unit)),
        "content" => s.get("content").unwrap_or_else(|| Value::str("")),
        _ => bail!("unknown method `{name}` on Span"),
    })
}

pub(super) fn line_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    Ok(match name {
        "width" => {
...
pub(super) fn cell_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    Ok(match name {
        "style" => with(s, "style", args.first().cloned().unwrap_or(Value::Unit)),
...
pub(super) fn row_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    let arg = args.first().cloned().unwrap_or(Value::Unit);
    Ok(match name {
        "style" => with(s, "style", arg),
        "height" => with(s, "height", arg),
        _ => bail!("unknown method `{name}` on Row"),
    })
}

pub(super) fn table_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    let arg = args.first().cloned().unwrap_or(Value::Unit);
    Ok(match name {
        "column_spacing" => with(s, "column_spacing", arg),
        "block" => with(s, "block", Value::some(arg)),
        "style" => with(s, "style", arg),
        "widths" => with(s, "widths", Value::vec(items(&arg))),
        _ => bail!("unknown method `{name}` on Table"),
    })
}

pub(super) fn block_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    let arg = args.first().cloned().unwrap_or(Value::Unit);
    Ok(match name {
        "title" => with(s, "title", Value::some(line_from(&arg))),
        "border_type" => with(s, "border_type", arg),
        "border_style" => with(s, "border_style", arg),
        "padding" => with(s, "padding", arg),
        "style" => with(s, "style", arg),
        "borders" => with(s, "bordered", Value::Bool(true)),
        _ => bail!("unknown method `{name}` on Block"),
    })
}

pub(super) fn sparkline_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    let arg = args.first().cloned().unwrap_or(Value::Unit);
    Ok(match name {
        "data" => with(s, "data", Value::vec(items(&arg))),
        "style" => with(s, "style", arg),
        "max" => with(s, "max", Value::some(arg)),
        _ => bail!("unknown method `{name}` on Sparkline"),
    })
}

pub(super) fn buffer_method(s: &StructData, name: &str, args: &[Value]) -> Result<Value> {
    Ok(match name {
...
pub(super) fn buffer_cell_method(s: &StructData, name: &str) -> Result<Value> {
    Ok(match name {
        "symbol" => s.get("symbol").unwrap_or_else(|| Value::str("")),
...
pub(super) fn constraint_variant(ty: &str, func: &str, args: &[Value]) -> Option<Value> {
    if ty != "Constraint" {
        return None;
    }
    ...
    let c = match func {
        "Length" => ratatui::layout::Constraint::Length(u16::try_from(n).unwrap_or(0)),
        "Min" => ratatui::layout::Constraint::Min(u16::try_from(n).unwrap_or(0)),
        "Max" => ratatui::layout::Constraint::Max(u16::try_from(n).unwrap_or(0)),
        "Percentage" => ratatui::layout::Constraint::Percentage(u16::try_from(n).unwrap_or(0)),
        "Fill" => ratatui::layout::Constraint::Fill(u16::try_from(n).unwrap_or(0)),
        "Ratio" => ratatui::layout::Constraint::Ratio(
...
/// carry data.
pub(super) fn color_variant(ty: &str, func: &str, args: &[Value]) -> Option<Value> {
    if ty != "Color" {
        return None;
    }
    let at = |i: usize| -> u8 {

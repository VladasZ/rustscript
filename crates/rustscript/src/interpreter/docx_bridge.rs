...
fn make_wt(text: &str) -> Element {
    ...
    t.attributes.insert("xml:space".to_string(), "preserve".to_string());
...
fn set_run_text(run: &mut Element, text: &str) {
    run.children.retain(|n| !matches!(n, XMLNode::Element(e) if is_run_content(&qname(e))));
    if !text.is_empty() {
...
fn line_cell<'a>(line: &'a serde_json::Value, i: usize) -> &'a str {
    line.get(i).and_then(serde_json::Value::as_str).unwrap_or("")
}
...
fn do_edit(xml: &str, opts_json: &str) -> Result<String, String> {
    ...
    let date = opts.get("date").and_then(serde_json::Value::as_str).unwrap_or("");
    let invoice_num = opts.get("invoice_num").and_then(serde_json::Value::as_str).unwrap_or("");
    let due = opts.get("due_date").and_then(serde_json::Value::as_str).unwrap_or("");
    let subtotal = opts.get("subtotal").and_then(serde_json::Value::as_str).unwrap_or("");
    let empty: Vec<serde_json::Value> = Vec::new();
    let lines = opts.get("lines").and_then(serde_json::Value::as_array).unwrap_or(&empty);
    ...
    if tables.len() < 4 {
        return Err(format!("Template should have at least 4 tables (found {})", tables.len()));
    }
    ...
    if let XMLNode::Element(table) = &mut body.children[tables[2]] {
        ...
        if lines.len() > capacity {
            return Err(format!("Template has only {capacity} line slots; got {} lines", lines.len()));
        }
        ...
        for r in (lines.len() + 1)..rows.len() {
            if let XMLNode::Element(row) = &mut table.children[rows[r]] {
                let cells = child_elem_indices(row, "w:tc");
    ...
    let config = EmitterConfig::new().perform_indent(false).write_document_declaration(false);
    root.write_with_config(&mut buf, config).map_err(|e| e.to_string())?;

...
impl PInterp {
    ...
    pub(super) fn render_fmt(&self, chunk: &PChunk, spec: u16, regs: &[PValue]) -> Result<String> {
        ...
        let positional: Vec<PValue> = f.positional.iter().map(|r| regs[*r as usize].clone()).collect();
        let named: Vec<(&str, PValue)> =
            f.named.iter().map(|(n, r)| (n.as_str(), regs[*r as usize].clone())).collect();
        render_template(&f.template, &positional, &named)
    }

    // -- iteration ---------------------------------------------------------

    pub(super) fn into_iter_items(&self, v: PValue) -> Result<Vec<PValue>> {
        Ok(match v {
    ...
    pub(super) fn dispatch_call(self: &Arc<Self>, segs: &[String], args: Vec<PValue>) -> Result<PValue> {
        ...
        match (ns, last) {
            ...
            _ => {
                ...
                if let Some((recv, rest)) = args.split_first() {
                    ...
                    let name = MethodName { id: super::bytecode::BuiltinId::resolve(last), text: last.to_string() };
    ...
    pub(super) fn eval_method(
        ...
        match recv {
            ...
            PValue::Int(_) | PValue::Float(_) | PValue::Bool(_) | PValue::Char(_) => {
                scalar_method(recv, m)
            }
            other => bail!("method `{m}` on {} is not supported in tokio mode", other.type_name()),
        }
    }

    fn vec_method(self: &Arc<Self>, recv: &PValue, m: &str, args: &mut [PValue]) -> Result<PValue> {
        let PValue::Vec(items) = recv else { unreachable!() };
        Ok(match m {
            "push" => {
                items.lock().push(args.first().cloned().unwrap_or(PValue::Unit));
                PValue::Unit
            }
            ...
            "first" => items.lock().first().cloned().map_or_else(PValue::none, PValue::some),
            "last" => items.lock().last().cloned().map_or_else(PValue::none, PValue::some),
            "contains" => {
                let needle = args.first().cloned().unwrap_or(PValue::Unit);
                PValue::Bool(items.lock().iter().any(|v| v.eq_value(&needle)))
            }
            "iter" | "into_iter" | "to_vec" => PValue::vec(items.lock().clone()),
            "sort" => {
    ...
    fn command_method(self: &Arc<Self>, recv: &PValue, m: &str, args: &mut [PValue]) -> Result<PValue> {
        let PValue::Struct(s) = recv else { unreachable!() };
        match m {
            "arg" => {
...
fn one(args: Vec<PValue>) -> Result<PValue> {
    args.into_iter().next().ok_or_else(|| anyhow::anyhow!("expected one argument"))
}
    ...
    if capture {
        match cmd.output() {
            Ok(out) => PValue::ok(PValue::struct_of(
                "Output",
                [
                    ("status".into(), exit_status(out.status.success(), out.status.code())),
...
fn duration_value(millis: i64) -> PValue {
    PValue::struct_of("Duration", [("millis".into(), PValue::Int(millis))])
}
...
fn map_method(recv: &PValue, m: &str, args: &mut [PValue]) -> Result<PValue> {
    let PValue::Map(map) = recv else { unreachable!() };
    ...
    Ok(match m {
        ...
        "get" => {
            let k = key(args).ok_or_else(|| anyhow::anyhow!("invalid map key"))?;
            map.lock().get(&k).cloned().map_or_else(PValue::none, PValue::some)
        }
        "contains_key" => {
            let k = key(args).ok_or_else(|| anyhow::anyhow!("invalid map key"))?;
        ...
        "remove" => {
            let k = key(args).ok_or_else(|| anyhow::anyhow!("invalid map key"))?;
            map.lock().shift_remove(&k).map_or_else(PValue::none, PValue::some)
        }
...
fn scalar_method(recv: &PValue, m: &str) -> Result<PValue> {
    Ok(match m {
        "abs" => match recv {
        ...
        _ => bail!("method `{m}` on {} is not supported in tokio mode", recv.type_name()),
    })
}

fn enum_method(recv: &PValue, m: &str, args: &mut [PValue]) -> Result<PValue> {
    let PValue::Enum { enum_name, variant, data } = recv else { unreachable!() };
    ...
    Ok(match m {
        ...
        "unwrap_or_default" => {
            if matches!(&**variant, "Some" | "Ok") { payload() } else { PValue::Unit }
        }
        ...
        "ok" => {
            if &**variant == "Ok" { PValue::some(payload()) } else { PValue::none() }
        }

...
Names resolve once, at compile time, and the VM works on ids. An enum value
carries a shared `EnumDef` and a variant index, so `Some(x)` and a user
variant build in place and a match compares two integers. A struct field
access caches its slot per site. Method names are `BuiltinId`s, generated by
`build.rs` from `method_names.txt`, and every bridge dispatches on them.
Path calls and path values like `fs::read_to_string` or `u32::MAX` are
`PathId`s from `path_names.txt`, matched by the longest suffix the table
lists, so `std::fs::read_to_string` and `fs::read_to_string` are the same id.
The script's own `impl` methods, operator traits, `Display`, `Drop`, and
`Iterator::next` are found by type id and method id in `interpreter/impls.rs`.
A name the tables do not know, a user function by bare name or a method
reference like `str::trim`, is looked up by name on a cold path. Adding a
method or path to the interpreter means adding its name to the table and
the arm that implements it, a name missing from the table is a compile
error.
